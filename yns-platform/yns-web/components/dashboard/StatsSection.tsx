...
export function StatsSection({ stats }: StatsSectionProps) {
  ...
  return (
    <section className="grid rounded-[14px] border border-[#e7dbd0] bg-white px-3 py-3 shadow-[0_2px_5px_rgba(78,45,31,0.05)] md:grid-cols-3 md:px-5">
      {items.map((item, index) => (
        <div
          key={item.label}
          className={`flex min-w-0 items-center gap-3 px-2 py-2 ${index > 0 ? 'border-t border-[#eadfd7] md:border-l md:border-t-0 md:pl-5' : ''}`}
        >
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs font-medium leading-4 text-[#8a7c75]">{item.label}</p>
          </div>
        </div>
      ))}
    </section>
