      ...
      <div className="min-h-[calc(100svh-4rem)] px-4 py-8 sm:px-6 lg:min-h-screen lg:pl-[220px]">
        <div className="mx-auto w-full max-w-6xl space-y-5 lg:px-6">
          <div className="h-28 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-4 text-sm font-bold text-slate-500">Loading session history...</p>
          </div>
  ...
  if (status === 'error' || !session) {
    return (
      <DetailState
        ...
        action={
          <Link href="/history" className={buttonVariants({ variant: 'outline', className: 'w-full rounded-lg font-bold' })}>
            Back to History
          </Link>
        }
      />
  ...
  if (session.turns.length === 0) {
    return (
      <DetailState
        ...
        action={
          <Link href="/history" className={buttonVariants({ className: 'w-full rounded-lg font-bold' })}>
            Back to History
          </Link>
        }
      />
        ...
        <div className="mx-auto w-full max-w-6xl space-y-6 lg:px-6">
          <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-center gap-2 text-sm font-bold text-slate-500">
              <Link href="/history" className="transition hover:text-slate-950">
                History
              </Link>
              <ChevronRight className="h-4 w-4 text-slate-400" aria-hidden="true" />
              <span className="text-slate-950">{formatSessionType(session.session_type)} Interview</span>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Badge className="gap-2 border-0 bg-indigo-50 px-3 py-1 text-indigo-700">
          ...
          <section className="space-y-4">
            <div>
              <h2 className="text-xl font-extrabold text-slate-950">Question by Question</h2>
              <p className="mt-1 text-sm font-semibold text-slate-500">One review card per question in this session.</p>
            </div>
          ...
          <footer className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <Link href="/history" className={buttonVariants({ variant: 'outline', className: 'rounded-lg font-bold' })}>
              Back to History
            </Link>
